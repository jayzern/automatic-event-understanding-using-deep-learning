...
outdir="/media/ymarton/HDD2T/data/rw-eng/ukwac-proc-out-20190223-mrft/"
start = 0
end = sys.maxint
...
#            cmd = "time -p %s/ukwac_converter.py -f %s -o %s -c" % (mydir, infilename, outdir) # currently he_srl + morfette
#            cmd = "%s/ukwac_converter.py -f %s -o %s -c" % (mydir, infilename, outdir) # currently he_srl + morfette
            cmd = "time -p %s/ukwac_converter-mrft.py -f %s -o %s -c" % (mydir, infilename, outdir)
            try:
                sb.check_call([cmd], shell=True)
            except sb.CalledProcessError as e:
                errmsg = "ERROR: calling ukwac batch tagger binary with %s" % cmd
                print(errmsg)
                sys.exit(errmsg)
                raise
