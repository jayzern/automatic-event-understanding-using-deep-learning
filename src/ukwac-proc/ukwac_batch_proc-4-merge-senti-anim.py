...
outdir="/media/ymarton/HDD2T/data/rw-eng/ukwac-proc-out-20200229-merged/"
start = 0
end = sys.maxint
...
#                             [join(inlist, f) for f in os.listdir(inlist) if isfile(join(inlist, f))] \
        ...
        if start <= i <= end:
            ...
            cmd = "python %s/ukwac_converter-merger.py -f %s  -o %s  -c" % (mydir, infilename, outdir) \
                  ...
                  + '["' + senti_with_prev_merge_dir + '",["pre-tokenized","morfette","senti-anim-wn","spacy","he_srl"]]]\'' \
                  + " | gzip -9 > %s" % logfn
            try:
...
# for i in {301..304} 307 308 311 {328..333} 338 340 341 349 352 353 358 366 374 251 253 261 265 267 {279..285}; do ~/src/sem-fit/ukwac-proc/ukwac_batch_proc-4-merge-senti-anim.py $i $i; done
