...
#    clean_data = UkwacTools.clean_bad_chars(data)
    clean_data = UkwacTools.clean_bad_chars(data).replace('\t',' ')
    ...
    print("sents len: %d" % len(sents))
    sents = UkwacTools.reduce_contr('\t'.join(sents)).split('\t')
    print("after reduce_contr: sents len: %d" % len(sents))
    ...
    if len(sents) == 0 or sents[0] == "":
        ...
        sents = rtokens.findall(clean_data)
        print("after len(sents) == 0: sents len: %d" % len(sents))
        sents = UkwacTools.reduce_contr('\t'.join(sents)).split('\t')
        print("len(sents) == 0 and reduce_contr: sents len: %d" % len(sents))
    if len(ids) == 0:
#        rtext_id = re.compile(r'(?:<text |<rawsent>)[\s\r\n]*(.*?)(?:/>|</rawsent>)', re.MULTILINE)
        rtext_id = re.compile(r'(<text |<rawsent>)[\s\r\n]*(.*?)[\s\r\n]*(?:/>|</rawsent>)', re.MULTILINE)
        ids_tmp = rtext_id.findall(clean_data)
        print("ids_tmp len: %d" % len(ids_tmp))
        ids = []
        last_id = ""
...
#                last_id = ids_tmp[i]
            if ids_tmp[i][0] == "<text ":   
                last_id = ids_tmp[i][1]
                #print last_id
#                print (" ids_tmp[%d]='%s';\n ids_tmp[%d]='%s' -- skipping\n" %(i-1,ids_tmp[i-1], i,ids_tmp[i]))
                continue #i += 1
            ids.append(last_id)
        print("ids len: %d" % len(ids))
        print("First id: '%s'" % ids[0])
        print("Last id: '%s'" % ids[-1])
    if len( orig_filenames) == 0:
        orig_filenames = ids
        
    if len(sents) != len(ids) or len(ids) != len( orig_filenames):
        print "DBG: sents=%s...%s\n ids=%s...%s\n orig_filenames=%s...%s\n" % (sents[:10],sents[-10:], ids[:10],ids[-10:], orig_filenames[:10],orig_filenames[-10:])
