...
NUM_TRAIN_FILES =   1.0  #0.1     # 0.01                  # 40 #400 #36 #None  # None for all except dev and test files taken first; fraction for uniform sampling fraction of the files not in dev or test; 1.0 for all except any dev or test
...
OUTPUT_PATH = '/home/ymarton/data/rw-eng/ukwac-proc-out-preproc4DL-20190929/exp7_%s-%d-%d-%s-%s/' % (NUM_TRAIN_FILES, len(NUM_DEV_FILES), len(NUM_TEST_FILES), ("FrAn" if USE_FRAMES_AND_ANIM else "NoFrAn"), CODE_VER)
...
#    unk_anim_id = len(anim_vocabulary)
...
def convert_file(file, dataset_type,
    ...
    for s in xml.findall("s"):
        ...
        for p in s.findall("predicate"):
            ...
            if USE_FRAMES_AND_ANIM:
                sample = dict((r, (missing_word_id,missing_anim_id,missing_frame_id)) for r in (list(role_vocabulary.values()) + [unk_role_id]))
            else:
                sample = dict((r, missing_word_id) for r in (list(role_vocabulary.values()) + [unk_role_id]))
...
#                    role = ROLE_OTHER
                ...
                if USE_FRAMES_AND_ANIM:
                    ...
                    if (0 <= head_rel_idx < len(arg_tokens)):
                        try:
                            if role == ROLE_PRD:
                                frame_name = arg_tokens[head_rel_idx].split('/')[16]
                            else:
                                anim = arg_tokens[head_rel_idx].split('/')[14]
                                ne   = arg_tokens[head_rel_idx].split('/')[12]
                                if (ne == ANIM_NE_PERSON) or (arg_head in ("i","me","myself", "we","us","ourselves", "you","you4rself", "he","him", "himself", "she", "her", "herself")):
                                    anim = ANIM_HMN
                        except:
                            pass
...
#    all_files = glob.glob(FILES_PATH)
    ...
    role_vocabulary = TEST_ROLES if (CODE_VER == "v1") else TRAIN_ROLES
    ...
    if USE_FRAMES_AND_ANIM:
        frame_vocabulary = build_frame_vocabulary()
        unk_anim_id = len(ANIMS)
        unk_frame_id= len(frame_vocabulary)
    ...
    write_description(TEST_ROLES,  ANIMS,       word_vocabulary, frame_vocabulary,
                      unk_role_id, unk_anim_id, unk_word_id,     unk_frame_id,
                      missing_role_id, missing_anim_id, missing_word_id, missing_frame_id,
