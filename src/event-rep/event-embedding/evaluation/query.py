...
def query(model_name, experiment_name, inputs, target):
    ...
    raw_words = dict((reverse_role_vocabulary[r], reverse_vocabulary[net.missing_word_id]) for r in list(net.role_vocabulary.values()))
    ...
    # n = int(sys.argv[3])    
    t_r = [net.role_vocabulary.get(r, net.unk_role_id) for r in list(target.keys())]
    t_w = [net.word_vocabulary.get(w, net.unk_word_id) for w in list(target.values())]

    input_roles_words = {}
    for r, w in list(raw_words.items()):
    ...
    for i, t_w_i in enumerate(resultlist):
        ...
        fb = numpy.floor(n)
        hb = n % 2
        print("{:<5} {:7.6f} {:<20} ".format(i+1, float(p), reverse_vocabulary[int(t_w_i)]) + "\u2588" * int(fb) + "\u258C" * int(hb))
