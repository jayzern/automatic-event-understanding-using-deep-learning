...
from tensorflow.keras.layers import Input, Embedding, Dropout, Dense, Lambda, Add, Multiply, Masking, Flatten, Concatenate
...
class MTRFv4RofBeg_v2(GenericModel):
    ...
    def __init__(self, n_word_vocab=50001, n_role_vocab=7, n_factors_emb=300, n_hidden=300, word_vocabulary=None, role_vocabulary=None, 
        ...
        super(MTRFv4RofBeg_v2, self).__init__(n_word_vocab, n_role_vocab, n_factors_emb, n_hidden, word_vocabulary, role_vocabulary, 
        ...
        # flatten
        flattened = Flatten(name = "flattened")(residual_0)

        # one-hot encoding of target role and word
        target_word_ohe = squeeze(K.one_hot(target_word, n_word_vocab), 1)
        target_role_ohe = squeeze(K.one_hot(target_role, n_role_vocab), 1)

        # concatenate
        word_concat = Concatenate(name = "word_concat")([target_word_ohe, flattened])
        role_concat = Concatenate(name = "role_concat")([target_role_ohe, flattened])

        # dense layer
        word_dense = Dense(n_factors_emb, 
            activation='linear', 
            input_shape=(input_length * n_factors_emb + n_word_vocab, ),
            )(word_concat)

        role_dense = Dense(n_factors_emb, 
            activation='linear', 
            input_shape=(input_length * n_factors_emb + n_role_vocab, ),
            )(role_concat)

        # non-linearity
        word_dense = PReLU(alpha_initializer='ones')(word_dense)
        role_dense = PReLU(alpha_initializer='ones')(role_dense)

        # softmax output layer
        target_word_output = Dense(n_word_vocab, 
            activation='softmax', 
            input_shape=(n_factors_emb, ), 
            name='softmax_word_output')(role_dense)

        # softmax output layer
        target_role_output = Dense(n_role_vocab, 
            activation='softmax', 
            input_shape=(n_factors_emb, ), 
            name='softmax_role_output')(word_dense)
