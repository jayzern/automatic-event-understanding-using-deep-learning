...
from tensorflow.keras.layers import Input, Embedding, Dropout, Dense, Lambda, Add, Multiply, Masking, RNN, LSTM, Bidirectional, Flatten, Activation, RepeatVector, Permute, Concatenate
...
class MTRFv4RofSeqTargAt(GenericModel):
    ...
    def __init__(self, n_word_vocab=50001, n_role_vocab=7, n_factors_emb=300, n_hidden=300, word_vocabulary=None, role_vocabulary=None,
        ...
        super(MTRFv4RofSeqTargAt, self).__init__(n_word_vocab, n_role_vocab, n_factors_emb, n_hidden, word_vocabulary, role_vocabulary,
        ...
        activations = Add(name='residual_0')([product, lin_proj2])
        ...
        # One-hot encoding of target role and word
        targ_word_ohe = squeeze(K.one_hot(target_word, n_word_vocab), 1)
        targ_role_ohe = squeeze(K.one_hot(target_role, n_role_vocab), 1)

        # Concatenate target role and target word OH with activations
        targ_word_ohe = RepeatVector(activations.shape[1])(targ_word_ohe)
        targ_role_ohe = RepeatVector(activations.shape[1])(targ_role_ohe)
        targ_word_conc_activations = Concatenate()([activations, targ_word_ohe])
        targ_role_conc_activations = Concatenate()([activations, targ_role_ohe])

        targ_word_attention = Dense(1, activation = 'tanh')(targ_word_conc_activations)
        targ_word_attention = Flatten()(targ_word_attention)
        targ_word_attention = Activation('softmax')(targ_word_attention)
        targ_word_attention = RepeatVector(n_factors_emb)(targ_word_attention)
        targ_word_attention = Permute([2,1])(targ_word_attention)

        targ_role_attention = Dense(1, activation = 'tanh')(targ_role_conc_activations)
        targ_role_attention = Flatten()(targ_role_attention)
        targ_role_attention = Activation('softmax')(targ_role_attention)
        targ_role_attention = RepeatVector(n_factors_emb)(targ_role_attention)
        ...
        targ_role_context_embedding = Lambda(lambda xin: K.sum(xin, axis=-2), output_shape=(n_factors_emb,), name='context_embedding2')(targ_role_context_embedding)

        # print(K.int_shape(context_embedding))

        # target word hidden layer
        tw_hidden = target_word_hidden(targ_role_context_embedding, target_role, n_word_vocab, n_role_vocab, glorot_uniform(), n_hidden, n_hidden)

        # target role hidden layer
        tr_hidden = target_role_hidden(targ_word_context_embedding, target_word, n_word_vocab, n_role_vocab, glorot_uniform(), n_hidden, n_hidden,
