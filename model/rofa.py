...
from tensorflow.keras.layers import Input, Embedding, Dropout, Dense, Lambda, Multiply, Masking, PReLU
...
class MTRFv4(GenericModel):
    ...
    """
    def __init__(self, n_word_vocab=50001, n_role_vocab=7, n_factors_emb=300, n_hidden=300, word_vocabulary={}, role_vocabulary={}, 
        ...
        embedding_layer = factored_embedding(input_words, input_roles, n_word_vocab, n_role_vocab, GlorotUniform(), 
        ...
        context_embedding = Lambda(lambda x: tf.keras.backend.mean(x, axis=1), 
            name='context_embedding',
            output_shape=(n_hidden,))(non_linearity)
            
        # target word hidden layer
        tw_hidden = target_word_hidden(context_embedding, target_role, n_word_vocab, n_role_vocab, GlorotUniform(), n_hidden, n_hidden, 
            using_dropout=using_dropout, dropout_rate=dropout_rate)

        # target role hidden layer
        tr_hidden = target_role_hidden(context_embedding, target_word, n_word_vocab, n_role_vocab, GlorotUniform(), n_hidden, n_hidden, 
            using_dropout=using_dropout, dropout_rate=dropout_rate)
