...
from tensorflow.keras.layers import Embedding, Dropout, Dense, Lambda, Add, Multiply, Dot, Masking, LSTM, GRU
...
def target_word_hidden(inputs, target_role, n_word_vocab, n_role_vocab, emb_init, 
    ...
    target_role_embedding = Lambda(lambda x: tf.keras.backend.sum(x, axis=1),
...
def target_role_hidden(inputs, target_word, n_word_vocab, n_role_vocab, emb_init, 
    ...
    # reduce dimension of tensor from 3 to 2
    target_word_embedding = Lambda(lambda x: tf.keras.backend.sum(x, axis=1),
...
def input_hidden(input_words, input_roles, n_word_vocab, n_role_vocab, emb_init, missing_word_id, 
    ...
    if a_target:
        ...
        context_hidden = Lambda(lambda x: tf.keras.backend.sum.sum(x, axis=1), 
            name='context_hidden',
